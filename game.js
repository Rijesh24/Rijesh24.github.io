...
// Draw both eyes regardless of direction
ctx.beginPath();
ctx.arc(player.x * gridSize + gridSize / 1.6, player.y * gridSize + gridSize / 2, gridSize / 6, 0, 2 * Math.PI);
ctx.arc(player.x * gridSize + gridSize / 3.3, player.y * gridSize + gridSize / 2, gridSize / 6, 0, 2 * Math.PI);
ctx.fill();
      ...
      function HitWall() {
  if (
    player.x < 1 || player.x > tileCount - 2 || player.y < 1 || player.y > tileCount - 2
  ) {
    game.reset();
  }
